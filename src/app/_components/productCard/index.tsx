...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <div
      key={product.id}
      className="relative flex justify-center flex-col items-center cursor-pointer text-center w-[285px] h-[446px] m-2 transition-all duration-300 group"
      onClick={handleClick}
    ...
    >
      <div className="relative w-full h-[301px]">
        ...
        />
        <div
          className={`absolute top-5 right-5 flex rounded-full w-10 h-10 items-center justify-center text-white ${
            product.new
              ? "bg-[#2EC1AC]"
        ...
        <div
          className={`absolute inset-0 bg-[#3A3A3A] transition-opacity duration-300 ${
      ...
      <div
        className={`text-left w-full h-[200px] p-4 transition-all duration-300 ${
          isHovered ? "bg-[#525252]" : "bg-[#F4F5F7]"
        }`}
      >
